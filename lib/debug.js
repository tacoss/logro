...
const showFulldate = process.argv.indexOf('--full') !== -1;
const showISODate = process.argv.indexOf('--iso') !== -1;
const noColor = process.argv.indexOf('--no-color') !== -1;

function format(message, data, now) {
  const time = new Date(now || Date.now())[showISODate ? 'toISOString' : 'toLocaleString']();
  const date = (showFulldate || showISODate) ? time : time.split(' ')[1];
